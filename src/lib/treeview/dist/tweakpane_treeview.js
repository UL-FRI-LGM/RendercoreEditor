...
const cn = ClassName('trw');
class TreeView {
    constructor(doc, config) {
        this.draggedNode_ = null;
        this.selectedNodeElement_ = null;
        this.selectedNode = null;
        this.onValueChange_ = this.onValueChange_.bind(this);
        this.toggleNode_ = this.toggleNode_.bind(this);
        this.isParentOfNode = this.isParentOfNode.bind(this);
        this.selectNode_ = this.selectNode_.bind(this);
        this.findNodesByValues = this.findNodesByValues.bind(this);
        this.addListenersToRoot_ = this.addListenersToRoot_.bind(this);
        this.props_ = config.props;
        this.element = doc.createElement('div');
        if (config.maxheight) {
            this.element.style.height = config.maxheight;
        }
        this.element.classList.add(cn());
        config.viewProps.bindClassModifiers(this.element);
        this.hiddenRoot = doc.createElement('div');
        this.hiddenRoot.classList.add(cn('root'));
        this.hiddenRoot.textContent = "\u200b";
        this.element.append(this.hiddenRoot);
        this.addListenersToRoot_();
        this.renderTree_(this.props_.get('options'), this.element);
        this.props_.get('options');
        config.value.emitter.on('change', this.onValueChange_);
        this.value_ = config.value;
        this.update_();
    }
    renderTree_(nodes, parentElement, ignoreList = new Set()) {
        //const tree = this.props_.get('options');
        // Clear the parent element to start fresh
        this.removeChildElementsExceptFirst(parentElement);
        const allChildren = new Set();
        nodes.forEach((node) => {
            if (node.children && node.children.length > 0) {
                for (let i = 0; i < node.children.length; i++) {
                    if (!(ignoreList === null || ignoreList === void 0 ? void 0 : ignoreList.has(node.children[i]))) {
                        allChildren.add(node.children[i]); // Add each child value to the array
                    }
                }
            }
        });
        nodes.forEach((node) => {
            var _a;
            if (allChildren.has(node.value)) {
                return;
            }
            const nodeElement = this.createNodeElement_(node);
            if (node == this.selectedNode) {
                this.selectNode_(nodeElement, null);
            }
            parentElement.appendChild(nodeElement);
            if (node.children && node.children.length > 0) {
                const childContainer = document.createElement('div');
                childContainer.classList.add(cn('children'));
                childContainer.style.display = 'block';
                parentElement.appendChild(childContainer); // Append the container after the node element
                // Event listener to toggle visibility
                (_a = nodeElement.querySelector('.' + cn('toggle'))) === null || _a === void 0 ? void 0 : _a.addEventListener('click', () => {
                    this.toggleNode_(childContainer, nodeElement);
                });
                const childrenToRender = this.findNodesByValues(node.children);
                const newChildren = new Set(node.children);
                newChildren.forEach((item) => { ignoreList.add(item); });
                this.renderTree_(childrenToRender, childContainer, ignoreList);
            }
        });
    }
    addListenersToRoot_() {
        this.hiddenRoot.addEventListener('dragover', (event) => {
            event.preventDefault(); // Necessary to allow dropping
            this.hiddenRoot.classList.add(cn('dragover'));
        });
        this.hiddenRoot.addEventListener('dragleave', () => {
            this.hiddenRoot.classList.remove(cn('dragover'));
        });
        this.hiddenRoot.addEventListener('drop', (event) => {
            event.preventDefault();
            this.hiddenRoot.classList.remove(cn('visible'));
            if (this.draggedNode_) {
                this.handleDrop_(this.draggedNode_, null);
                this.draggedNode_ = null;
            }
        });
    }
    createNodeElement_(node) {
        const nodeElement = document.createElement('div');
        nodeElement.classList.add(cn('node'));
        const toggleElement = document.createElement('span');
        toggleElement.classList.add(cn('toggle'));
        toggleElement.textContent = node.children && node.children.length > 0 ? '+' : '';
        nodeElement.appendChild(toggleElement);
        const textElement = document.createElement('span');
        textElement.classList.add(cn('text'));
        textElement.textContent = node.text;
        nodeElement.appendChild(textElement);
        nodeElement.draggable = true; // Make the node draggable
        nodeElement.style.touchAction = "pan-y";
        nodeElement.addEventListener('dragstart', (event) => {
            var _a;
            this.draggedNode_ = node;
            (_a = event.dataTransfer) === null || _a === void 0 ? void 0 : _a.setData('text/plain', nodeElement.dataset.value || '');
            nodeElement.classList.add('dragging');
            this.hiddenRoot.classList.add(cn('visible'));
        });
        ...
        nodeElement.addEventListener('dragover', (event) => {
            event.preventDefault(); // Necessary to allow dropping
            nodeElement.classList.add(cn('dragover'));
        });
        nodeElement.addEventListener('dragleave', () => {
            nodeElement.classList.remove(cn('dragover'));
        });
        nodeElement.addEventListener('drop', (event) => {
            ...
            this.hiddenRoot.classList.remove(cn('visible'));
            this.props_.get('options');
            if (this.draggedNode_ && node != null) {
                this.handleDrop_(this.draggedNode_, node);
                this.draggedNode_ = null;
            }
        });
        nodeElement.addEventListener('click', () => {
            this.selectedNode = node;
            this.selectNode_(nodeElement, node);
        });
        return nodeElement;
    }
    handleDrop_(draggedNode, targetNode) {
        //console.log("handling drop    ")
        if (targetNode == null) {
            const tree = this.props_.get('options');
            const prevParent = this.findParentByChildValue(draggedNode.value);
            if (prevParent != null && prevParent.children != null) {
                prevParent.children.splice(prevParent.children.indexOf(draggedNode.value), 1);
                console.log(prevParent.children);
            }
            this.props_.set('options', tree);
            this.value_.setRawValue(this.value_.rawValue, { forceEmit: true, last: false });
            this.renderTree_(tree, this.element);
            return;
        }
        else {
            //Check if dragged onto self.
            if (targetNode.value == draggedNode.value) {
                //console.log("Cannot drag object onto self")
                return;
            }
            //check if parent is being dragged onto its' child
            const check = this.isParentOfNode(draggedNode, targetNode);
            if (check) {
                //console.log("Cannot drag parent onto child")
                return;
            }
            const tree = this.props_.get('options');
            // Remove dragged node as child from previous parent node if had one
            const prevParent = this.findParentByChildValue(draggedNode.value);
            if (prevParent != null && prevParent.children != null) {
                prevParent.children.splice(prevParent.children.indexOf(draggedNode.value));
            }
    ...
    toggleNode_(childContainer, nodeElement) {
        const isVisible = childContainer.style.display !== 'none';
        childContainer.style.display = isVisible ? 'none' : 'block';
        const toggleElement = nodeElement.querySelector('.' + cn('toggle'));
        if (toggleElement) {
            toggleElement.textContent = isVisible ? '+' : '-';
        }
    }
    isParentOfNode(A, B) {
        const recursiveChildCheck = (A, B, currentNode) => {
            //console.log(currentNode.value)
            if (currentNode.value == B.value && A.value != currentNode.value) {
                return true;
            }
            //Incorrect node, check if has child.
            if (!currentNode.children || currentNode.children.length === 0) {
                return false;
            }
            //Has child, iterate and check
            var ret = false;
            const children = this.findNodesByValues(currentNode.children);
            children.forEach((node) => {
                if (recursiveChildCheck(A, B, node)) {
                    ret = true;
                }
            });
            return ret;
        };
        return recursiveChildCheck(A, B, A);
    }
    findNodesByValues(searchValues) {
        const tree = this.props_.get('options');
        const foundNodes = [];
        tree.forEach((node) => {
    ...
    findParentByChildValue(searchValue) {
        var _a;
        const tree = this.props_.get('options');
        for (var i = 0; i < tree.length; i++) {
            const node = tree[i];
            if ((_a = node.children) === null || _a === void 0 ? void 0 : _a.includes(searchValue)) {
                return node;
            }
        }
        return null;
    }
    removeChildElementsExceptFirst(element) {
        while (element.children.length > 1) {
            element.removeChild(element.children[1]);
        }
    }
    selectNode_(nodeElement, node) {
        // Deselect previous
        if (this.selectedNodeElement_ && node) {
            this.selectedNodeElement_.classList.remove(cn('selected'));
            this.selectedNode = node;
        }
        // Select the new node
        this.selectedNodeElement_ = nodeElement;
        this.selectedNodeElement_.classList.add(cn('selected'));
        if (node) {
            this.value_.setRawValue(node.value, { forceEmit: true, last: true });
        }
    }
    update_() {
    }
...
class TreeViewController {
    constructor(doc, config) {
        ...
        this.viewProps = config.viewProps;
        this.view = new TreeView(doc, {
            props: this.props,
            value: this.value,
            viewProps: this.viewProps,
            maxheight: this.maxheight,
        });
        this.view.value_.emitter.on('change', this.onSelectChange_);
    }
    onSelectChange_(e) {
        this.value.rawValue = this.view.value_.rawValue;
    }
    importProps(state) {
        return importBladeState(state, null, (p) => ({
            options: p.required.custom(parseListOptions),
...
function parseTreeOptions(value) {
    var _a;
    const p = MicroParsers;
    // Helper function to parse TreeNode<T> in array style
    function parseArrayStyleTreeNode(node) {
        return p.required.object({
            text: p.required.string,
            value: p.required.raw,
            children: p.optional.array(p.required.raw), // Add this line
        })(node);
    }
    // Parsing ArrayStyleTreeOptions<T>
    if (Array.isArray(value)) {
        return (_a = p.required.array(parseArrayStyleTreeNode)(value)) === null || _a === void 0 ? void 0 : _a.value;
    }
    return undefined;
}
function normalizeTreeOptions(options) {
    ...
    Object.keys(options).forEach((text) => {
        ...
        let item = {
            ...
            children: options[text].children ? options[text].children : undefined,
        };
        items.push(item);
    });
    return items;
...
class TreeBladeApi extends BladeApi {
    /**
     * @hidden
     */
    constructor(controller) {
...
//
const TreeViewBladePlugin = (function () {
    return {
        id: 'treeview',
        ...
        accept(params) {
            ...
            const result = parseRecord(params, (p) => ({
                ...
                options: p.required.custom(parseTreeOptions),
                value: p.required.raw,
                view: p.required.constant('treeview'),
                height: p.optional.string,
                label: p.optional.string,
            }));
            if (!result) {
                return null;
        ...
        controller(args) {
            const lc = new ListConstraint(normalizeTreeOptions(args.params.options));
            const value = createValue(args.params.value, {
                constraint: lc,
            });
            const ic = new TreeViewController(args.document, {
                props: new ValueMap({
                    options: lc.values.value('options'),
                }),
                value: value,
                viewProps: args.viewProps,
                maxheight: args.params.height,
            });
        ...
        api(args) {
            ...
            if (!(args.controller.valueController instanceof TreeViewController)) {
                return null;
            }
            return new TreeBladeApi(args.controller);
        },
    };
})();
...
const css = '.tp-trwv{display:block;width:100%;overflow-x:scroll;color:var(--mo-fg)}.tp-trwv_node{cursor:pointer;margin:4px 0;padding-left:15px;position:relative}.tp-trwv_node_toggle{position:absolute;left:0px;top:0px;cursor:pointer}.tp-trwv_node_text{display:inline-block;font:var(--bs-ff);color:var(--lbl-fg)}.tp-trwv_node_dragging{opacity:.5}.tp-trwv_children{display:none;margin-left:15px;padding-left:10px;border-left:1px solid var(--lbl-fg)}.tp-trwv_dragover{border:2px dashed var(--lbl-fg)}.tp-trwv_selected{background-color:var(--btn-bg-a);color:var(--btn-fg)}.tp-trwv_root{max-height:20px;border:2px dashed var(--btn-bg);visibility:collapse}.tp-trwv_visible{visibility:visible}.tp-trwv::-webkit-scrollbar-thumb{background:var(--lbl-fg)}.tp-trwv::-webkit-scrollbar{background:var(--btn-fg);width:.5em;height:.5em;padding:1em;margin:1em auto}.tp-trwv::-webkit-scrollbar-corner{background:var(--btn-fg)}';
// Export your plugin(s) as a constant `plugins`
const plugins = [TreeViewBladePlugin];
