...
class CascadeView {
    constructor(doc, config) {
        ...
        this.top = config.top;
        this.element = doc.createElement('div');
        this.element.textContent = String(config.root);
        this.element.classList.add(cn('main'));
        this.element.classList.add("tp-btnv_b");
        config.viewProps.bindClassModifiers(this.element);
        this.initMenu(this.props_.get('options'));
    ...
    initMenu(options) {
        ...
        if (this.top != undefined) {
            menuContainer.style.top = this.top;
        }
        this.element.appendChild(menuContainer);
        //mouseOverFix.appendChild(menuContainer); 
        menuContainer.classList.add(cn("maindrop_container"));
        ...
        const topLevelItems = options.filter(option => !options.some(opt => { var _a; return (_a = opt.children) === null || _a === void 0 ? void 0 : _a.includes(option.value); }));
        topLevelItems.forEach(item => {
            const li = document.createElement('li');
            li.textContent = item.text;
            li.classList.add(cn('maindrop'));
            li.addEventListener("click", (e) => {
                e.stopPropagation();
                this.value_.setRawValue(item.value, { forceEmit: true, last: true });
            });
            // If the item has children, create a submenu
            if (item.children && item.children.length > 0) {
                const submenuContainer = document.createElement('div');
                this.createMenu(submenuContainer, item.children);
                //submenuContainer.appendChild(submenu);
                menuContainer.appendChild(submenuContainer);
                submenuContainer.classList.add(cn("rightdrop_container"));
    ...
    createMenu(parentContainer, items) {
        items.forEach(itemValue => {
            const item = this.findOptionByValue(itemValue, this.props_.get('options'));
            if (item) {
                const li = document.createElement('li');
                li.textContent = item.text;
                li.classList.add(cn('rightdrop'));
                li.addEventListener("click", (e) => {
                    e.stopPropagation();
                    this.value_.setRawValue(item.value, { forceEmit: true, last: true });
                });
                // If the item has children, create the submenu recursively
                if (item.children && item.children.length > 0) {
                    const submenuContainer = document.createElement('div');
                    this.createMenu(submenuContainer, item.children);
                    //submenuContainer.appendChild(submenu);
...
function parseCascadeOptions(value) {
    var _a;
    const p = MicroParsers;
...
function normalizeCascadeOptions(options) {
    if (Array.isArray(options)) {
        return options;
    }
    const items = [];
...
class CascadeApi extends BladeApi {
    /**
     * @hidden
     */
    constructor(controller) {
...
const CascadeMenuPlugin = (function () {
    return {
        id: 'cascadeMenu',
        ...
        accept(params) {
            ...
            const result = parseRecord(params, (p) => ({
                ...
                options: p.required.custom(parseCascadeOptions),
                value: p.optional.raw,
                view: p.required.constant('cascadeMenu'),
                root: p.required.string,
                label: p.optional.string,
                top: p.optional.string,
            }));
            if (!result) {
                return null;
            }
        ...
        controller(args) {
            const lc = new ListConstraint(normalizeCascadeOptions(args.params.options));
            const value = createValue(args.params.value, {
                constraint: lc,
            });
            const ic = new CascadeMenuController(args.document, {
                props: new ValueMap({
                    options: lc.values.value('options'),
                }),
                value: value,
        ...
        api(args) {
            ...
            if (!(args.controller.valueController instanceof CascadeMenuController)) {
                return null;
            }
            return new CascadeApi(args.controller);
        },
    };
})();

// The identifier of the plugin bundle.
const id = 'cascadeMenu';
// This plugin template injects a compiled CSS by @rollup/plugin-replace
// See rollup.config.js for details
const css = '.tp-csmv{-webkit-appearance:none;-moz-appearance:none;appearance:none;background-color:rgba(0,0,0,0);border-width:0;font-family:inherit;font-size:inherit;font-weight:inherit;margin:0;outline:none;padding:0}.tp-csmv{background-color:var(--in-bg);border-radius:var(--bld-br);box-sizing:border-box;color:var(--in-fg);font-family:inherit;height:var(--cnt-usz);line-height:var(--cnt-usz);min-width:0;width:100%}.tp-csmv:hover{background-color:var(--in-bg-h)}.tp-csmv:focus{background-color:var(--in-bg-f)}.tp-csmv:active{background-color:var(--in-bg-a)}.tp-csmv:disabled{opacity:.5}.tp-csmv{border-radius:var(--bld-br);cursor:pointer;font-weight:bold;font:var(--bs-ff);height:var(--cnt-usz);line-height:var(--cnt-usz);text-overflow:ellipsis;white-space:nowrap}.tp-csmv_main{width:100px;vertical-align:middle;padding-top:5%;padding:1%;text-align:center;background-color:var(--bs-bg) !important;color:var(--in-fg) !important;font:var(--bs-ff)}.tp-csmv_maindrop_container{position:absolute;top:100%;width:100px;display:"none";background-color:var(--bs-bg)}.tp-csmv_maindrop{position:relative;width:100px;height:20px;top:0;left:0;padding:10% 5% 5% 10%;visibility:visible;text-align:left;color:var(--in-fg);background-color:var(--bs-bg)}.tp-csmv_maindrop:hover{color:var(--btn-fg);background-color:var(--btn-bg-h)}.tp-csmv_maindrop:active{color:var(--btn-fg);background-color:var(--btn-bg-a)}.tp-csmv_rightdrop_container{position:absolute;left:100%;background-color:var(--bs-bg);top:0%}.tp-csmv_rightdrop{position:relative;width:100px;height:20px;top:0;text-align:left;left:0;padding:10% 5% 5% 10%;color:var(--in-fg);background-color:var(--bs-bg)}.tp-csmv_rightdrop:hover{color:var(--btn-fg);background-color:var(--btn-bg-h)}.tp-csmv_rightdrop:active{color:var(--btn-fg);background-color:var(--btn-bg-a)}';
// Export your plugin(s) as a constant `plugins`
const plugins = [CascadeMenuPlugin];
