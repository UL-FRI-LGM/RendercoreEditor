...
const MODE_REINHARD: f32 = 0.0;
const MODE_EXPOSURE: f32 = 1.0;
struct MODE_ {
	REINHARD: f32,
	EXPOSURE: f32,
};

const MODE: MODE_ = MODE_(0.0, 1.0);
...
fn main(input: v_VertexOutputFragmentInput) -> o_FragmentOutput {
	...
	var hdrColor: vec4<f32> = textureSample(u_MaterialTexture0, u_MaterialSampler0, input.v_uv).rgba; //input color

	var mapped: vec3<f32>;
	if (u_Material.MODE == MODE_REINHARD) {
		// Reinhard tone mapping
		mapped = hdrColor.rgb / (hdrColor.rgb + vec3<f32>(1.0));
	} else if (u_Material.MODE == MODE_EXPOSURE) {
