...
export class SpotLight extends PointLight {
    ...
    get innerCutoff() { return Math.acos(this.#innerCutoff); }
    set innerCutoff(innerCutoff) {
		this.#innerCutoff = Math.cos(innerCutoff);


		const instruction = this.instructionCache.get("INNER_CUTOFF");
		instruction.source.arrayBuffer.set([Math.cos(innerCutoff)]);

		this.dirtyCache.set("INNER_CUTOFF", instruction);
	}
    get outerCutoff() { return Math.acos(this.#outerCutoff); }
    set outerCutoff(outerCutoff) {
		this.#outerCutoff = Math.cos(outerCutoff);


		const instruction = this.instructionCache.get("OUTER_CUTOFF");
		instruction.source.arrayBuffer.set([Math.cos(outerCutoff)]);
