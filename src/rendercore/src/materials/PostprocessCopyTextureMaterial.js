...
export class PostprocessCopyTextureMaterial extends MeshMaterial {


	static DEFAULT = {
		TYPE: "PostprocessCopyTextureMaterial",
		NAME: "",

		RESOURCE_PACK: new ResourcePack({ name: `RP - ${PostprocessCopyTextureMaterial.name}` }),
		INSTRUCTION_CACHE: new InstructionCache(
			{ name: `IC - ${PostprocessCopyTextureMaterial.name}` },
			[
	...
	constructor(args = {}) {
		super(
			{
				...
				type: (args.type !== undefined) ? args.type : PostprocessCopyTextureMaterial.DEFAULT.TYPE,
				name: (args.name !== undefined) ? args.name : PostprocessCopyTextureMaterial.DEFAULT.NAME,

				resourcePack: (args.resourcePack !== undefined) ? args.resourcePack : PostprocessCopyTextureMaterial.DEFAULT.RESOURCE_PACK.clone(),
				instructionCache: (args.instructionCache !== undefined) ? args.instructionCache : PostprocessCopyTextureMaterial.DEFAULT.INSTRUCTION_CACHE.clone(),

				shaderPath: (args.shaderPath !== undefined) ? args.shaderPath : PostprocessCopyTextureMaterial.DEFAULT.SHADER_PATH,
				programName: (args.programName !== undefined) ? args.programName : PostprocessCopyTextureMaterial.DEFAULT.PROGRAM_NAME,
		...
		this.resourcePack.setResourceBindingInternal(
			...
			new ResourceBinding(
				{
					number: 0,
					arrayBuffer: new Float32ArrayT2({}, 1*4),
					
					resourceDescriptor: new BufferDescriptor(
						{
							label: "mesh copy material buffer",
							size: (1*4),
			...
			new ResourceBinding(
				{
					...
					resourceDescriptor: new TextureDescriptor(
						{
							label: "mesh copy material texture 0",
			...
			new ResourceBinding(
				{
					...
					resourceDescriptor: new SamplerDescriptor(
						{
							label: "mesh copy material sampler 0",
