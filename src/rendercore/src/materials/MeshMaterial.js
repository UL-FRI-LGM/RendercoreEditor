...
export class MeshMaterial extends Material { //mesh custom material
	...
	static DEFAULT = {
		...
		RESOURCE_PACK: new ResourcePack({ name: "RP - MESH MATERIAL" }),
		INSTRUCTION_CACHE: new InstructionCache(
			{ name: "IC - MESH MATERIAL" },
			[
				...Material.DEFAULT.INSTRUCTION_CACHE.clone(),
			]
		),
		...
		SHADER_PATH: "/src/rendercore/src/shaders/default",
		...
		super(
			{
				...
				resourcePack: (args.resourcePack !== undefined) ? args.resourcePack : MeshMaterial.DEFAULT.RESOURCE_PACK.clone(),
				instructionCache: (args.instructionCache !== undefined) ? args.instructionCache : MeshMaterial.DEFAULT.INSTRUCTION_CACHE.clone(),
			}
		);
	...
	setMapBinding(name, groupNumber, mapNumber, mapBinding, setInstruction = undefined) {
		this.maps.set(name, mapBinding);

		this.addSBFlag("MAPS");
		this.addSBValue("NUM_MAPS", this.maps.size);


		this.resourcePack.setMapBindingExternal(groupNumber, mapNumber, mapBinding, setInstruction);
		this.numMaps = this.maps.size;
	}
	removeMapBinding(name) {
		this.maps.delete(name);

		if (this.maps.size === 0) this.rmSBFlag("MAPS");
		this.addSBValue("NUM_MAPS", this.maps.size);
		this.numMaps = this.maps.size;
	}
	clearMapBindings() {
