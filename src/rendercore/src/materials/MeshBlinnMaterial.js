...
export class MeshBlinnMaterial extends MeshMaterial {
	
	
	static DEFAULT = {
		NAME: "",
		TYPE: "MeshBlinnMaterial",

		RESOURCE_PACK: new ResourcePack({ name: "RP - MESH BLINN MATERIAL" }),
		INSTRUCTION_CACHE: new InstructionCache(
			{ name: "IC - MESH BLINN MATERIAL" },
			[
				...MeshMaterial.DEFAULT.INSTRUCTION_CACHE.clone(),
			]
		),

		SHADER_PATH: "/src/rendercore/src/shaders/blinn/",
		PROGRAM_NAME: "blinn_smooth",
	};
	...
	constructor(args = {}) {
		super(
			{
				...
				name: (args.name !== undefined) ? args.name : MeshBlinnMaterial.DEFAULT.NAME,
				type: (args.type !== undefined) ? args.type : MeshBlinnMaterial.DEFAULT.TYPE,

				resourcePack: (args.resourcePack !== undefined) ? args.resourcePack : MeshBlinnMaterial.DEFAULT.RESOURCE_PACK.clone(),
				instructionCache: (args.instructionCache !== undefined) ? args.instructionCache : MeshBlinnMaterial.DEFAULT.INSTRUCTION_CACHE.clone(),

				shaderPath: (args.shaderPath !== undefined) ? args.shaderPath : MeshBlinnMaterial.DEFAULT.SHADER_PATH,
				programName: (args.programName !== undefined) ? args.programName : MeshBlinnMaterial.DEFAULT.PROGRAM_NAME,
			...
			new ResourceBinding(
				{
					...
					resourceDescriptor: new BufferDescriptor(
						{
							label: "mesh blinn material buffer",
							size: (4*4),
		...
		for (let i = 0; i < 4; i++) {
				...
				new ResourceBinding(
					{
						...
						resourceDescriptor: new TextureDescriptor(
							{
								label: `mesh blinn material texture ${i}`,
			...
			this.resourcePack.setResourceBindingValueInternal(
				...
				new TextureSetInstruction(
					{
						label: `mesh blinn material texture ${i} set`,
				...
				new ResourceBinding(
					{
						...
						resourceDescriptor: new SamplerDescriptor(
							{
								label: `mesh blinn material sampler ${i}`,
		...
		// 			resourceDescriptor: new TextureDescriptor(
		// 				{
		// 					label: "mesh blinn material texture array",
