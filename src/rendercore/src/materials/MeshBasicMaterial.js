...
export class MeshBasicMaterial extends MeshMaterial {
	...
	static DEFAULT = {
		...
		RESOURCE_PACK: new ResourcePack({ name: "RP - MESH BASIC MATERIAL" }),
		INSTRUCTION_CACHE: new InstructionCache(
			{ name: "IC - MESH BASIC MATERIAL" },
			[
		...
		super(
			{
				...
				resourcePack: (args.resourcePack !== undefined) ? args.resourcePack : MeshBasicMaterial.DEFAULT.RESOURCE_PACK.clone(),
				instructionCache: (args.instructionCache !== undefined) ? args.instructionCache : MeshBasicMaterial.DEFAULT.INSTRUCTION_CACHE.clone(),
		...
		for (let i = 0; i < 4; i++) {
				...
				new ResourceBinding(
					{
						...
						resourceDescriptor: new TextureDescriptor(
							{
								label: `mesh basic material texture ${i}`,
			...
			this.resourcePack.setResourceBindingValueInternal(
				...
				new TextureSetInstruction(
					{
						label: `mesh basic material texture ${i} set`,
				...
				new ResourceBinding(
					{
						...
						resourceDescriptor: new SamplerDescriptor(
							{
								label: `mesh basic material sampler ${i}`,
