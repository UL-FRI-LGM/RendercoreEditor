...
export class Camera extends Group {
	...
	constructor(args = {}) {
		...
		this.resourcePack.setResourceBindingInternal(
			...
			new ResourceBinding(
				{
					number: 0,
					arrayBuffer: new Float32Array(16*4),
	...
	set projectionMatrix(projectionMatrix) { 
		...
		const instruction_PMat = this.instructionCache.get("PMat");
		instruction_PMat.source.arrayBuffer.set(projectionMatrix.elements);

		this.resourcePack.setResourceBindingValueInternal(0, 0, instruction_PMat);


		this.projectionMatrixInverse.getInverse(projectionMatrix);

		const instruction_PMatInv = this.instructionCache.get("PMatInv");
		instruction_PMatInv.source.arrayBuffer.set(this.projectionMatrixInverse.elements);

		this.resourcePack.setResourceBindingValueInternal(0, 0, instruction_PMatInv);
