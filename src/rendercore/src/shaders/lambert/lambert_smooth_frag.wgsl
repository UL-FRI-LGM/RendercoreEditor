...
fn calculateDirectionalLight(light: DLight, normal: vec3<f32>) -> vec3<f32> {
...
fn calculatePointLight(light: PLight, position: vec4<f32>, normal: vec3<f32>) -> vec3<f32> {
...
fn calculateSpotLight(light: SLight, position: vec4<f32>, normal: vec3<f32>) -> vec3<f32> {
...
fn main(input: v_VertexOutputFragmentInput) -> o_FragmentOutput {
	let normal_viewspace: vec3<f32> = mix(-input.v_normal_viewspace.xyz, input.v_normal_viewspace.xyz, f32(input.front_facing));
