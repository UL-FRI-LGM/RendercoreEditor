...
fn calculateDirectionalLight(light: DLight) -> vec3<f32> {
	// Combine results
	...
	var diffuse: vec3<f32> = color * u_Material.diffuse.rgb;
...
fn calculateSpotLight(light: SLight, position: vec4<f32>) -> vec3<f32> {
...
fn main(input: i_VertexInput) -> v_VertexOutputFragmentInput {
	...
	for (var i_DLight: u32 = u32(0); i_DLight < u_Light.numDLights; i_DLight++) {
		...
		color += calculateDirectionalLight(DL);
	}
	...
	for (var i_SLight: u32 = u32(0); i_SLight < u_Light.numSLights; i_SLight++) {
		...
		color += calculateSpotLight(SL, position_viewspace);
	}


	return v_VertexOutputFragmentInput(
		position_clipspace,
