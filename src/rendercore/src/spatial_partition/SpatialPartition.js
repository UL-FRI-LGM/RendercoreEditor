...
export class SpatialPartition extends BoxFrame {
		...
		super(
			{
				...
				renderOrder: (args.renderOrder !== undefined) ? args.renderOrder : SpatialPartition.DEFAULT.RENDER_ORDER,

				parent: (args.parent !== undefined) ? args.parent : SpatialPartition.DEFAULT.PARENT,
				children: (args.children !== undefined) ? args.children : SpatialPartition.DEFAULT.CHILDREN.clone(false),
				ancestors: (args.ancestors !== undefined) ? args.ancestors : SpatialPartition.DEFAULT.ANCESTORS.clone(false),
				descendants: (args.descendants !== undefined) ? args.descendants : SpatialPartition.DEFAULT.DESCENDANTS.clone(false),

				resourcePack: (args.resourcePack !== undefined) ? args.resourcePack : SpatialPartition.DEFAULT.RESOURCE_PACK.clone(),
				instructionCache: (args.instructionCache !== undefined) ? args.instructionCache : SpatialPartition.DEFAULT.INSTRUCTION_CACHE.clone(),
				dirtyCache: (args.dirtyCache !== undefined) ? args.dirtyCache : SpatialPartition.DEFAULT.DIRTY_CACHE.clone(),
	...
	addClient(client) {
		return this.forEachPartitionElement((element, ke, nodes) => {
			return this.forEachBoundNode((node) => { return node.addClient(client); }, nodes, undefined);
		});
	}
	addClients(clients) {
	...
	removeClient(client) {
		return this.forEachPartitionElement((element, ke, nodes) => {
			return this.forEachBoundNode((node) => { return node.removeClient(client); }, nodes, undefined);
		});
	}
